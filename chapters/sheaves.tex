...
\begin{definition}[Stalks]
  ...
  We can also form the stalk of sheaves on the \'etale toplogy. Let $X$ be a scheme and $x$ a point of $X$. An \'etale neighborhood of $x$ is defined to be a pair $(U, u)$ together with an \'etale morphism $\varphi: U \to X$ such that $\varphi(u) = x$. A morphism of \'etale neighborhoods $f: (V,v) \to (U,u)$ is a morphism of $X$-schemes $f: V \to U$ such that $f(v) = u$. The 
\end{definition}
...
\begin{corollary}
  Every presheaf $\widehat{Z}$ represented by an $X$-scheme $Z$ given by $U \to \Hom_X(U,Z)$ is a sheaf. 
\end{corollary}
...
\begin{example}
	Let $R=\Z[x_z, \dots, x_n]/(f_1, \dots , f_m)$ and $X = \Spec(R)$. What are the $\Z$-valued points of $R$? Specifying a morphism from $\Spec(\Z)$ to $X$ is equivalent to specifying a morphism $\varphi: R \to \Z$, which is equivalent to specifying images $a_i$ of the generators $x_i$ of $R$ such that $f_j(a_1, \dots, a_n) = 0$ for all $j$. In other words, the $\Z$-valued points are the integral solutions to the equatiosn $f_j = 0$
\end{example}

\begin{example}
	Consider the scheme $\mu_n = \Spec(\Z[x]/(x^n-1))$. \end{example}
\begin{example}
...
\subsection{The category $BG$}
We now come to important and simple examples of Grothendieck toposes.
\begin{construction}
